 ...
 * @author Paul Ferlitz
 * @see INBTReader
 * @see de.pauleff.jnbt.api.INBTWriter
...
public class NBTFileFactory
{
     ...
     *
     * @param nbtFile The {@link File} to read NBT data from
     ...
     *
     * @param dis The {@link DataInputStream} containing NBT data
     ...
     *
     * @param nbtFile The {@link File} to write NBT data to
     * @return New {@link de.pauleff.jnbt.api.INBTWriter} ready to write data
     * @throws FileNotFoundException If the target file doesn't exist for compression detection
     */
    public static de.pauleff.jnbt.api.INBTWriter createWriter(File nbtFile) throws FileNotFoundException
    {
     ...
     * @return New {@link de.pauleff.jnbt.api.INBTWriter} ready to write compressed data
     */
    public static de.pauleff.jnbt.api.INBTWriter createWriter(File nbtFile, Compression_Types compression)
    {
     ...
     *
     * @param nbtFile The {@link File} to read
     * @return The root {@link de.pauleff.jnbt.api.ICompoundTag} containing all NBT data
     * @throws IOException If the file cannot be read or parsed
     */
    public static de.pauleff.jnbt.api.ICompoundTag readNBTFile(File nbtFile) throws IOException
    {
     ...
     *
     * @param nbtFile The {@link File} to write to
     * @param root    The {@link de.pauleff.jnbt.api.ICompoundTag} to write
     * @throws IOException If the file cannot be written
     */
    public static void writeNBTFile(File nbtFile, de.pauleff.jnbt.api.ICompoundTag root) throws IOException
    {
        try (de.pauleff.jnbt.api.INBTWriter writer = createWriter(nbtFile))
        {
            writer.write(root);
        } catch (FileNotFoundException e)
        {
            // If the file doesn't exist, create a new one with default compression = NONE
            try (de.pauleff.jnbt.api.INBTWriter writer = createWriter(nbtFile, Compression_Types.NONE))
            {
                writer.write(root);
            }
        }
     ...
     * @throws IOException If the file cannot be written
     */
    public static void writeNBTFile(File nbtFile, de.pauleff.jnbt.api.ICompoundTag root, Compression_Types compression) throws IOException
    {
     ...
     * @param destination The destination {@link File} to copy to
     * @throws IOException If the copy operation fails
     */
    public static void copyNBTFile(File source, File destination) throws IOException
    {
        de.pauleff.jnbt.api.ICompoundTag data = readNBTFile(source);
     ...
     *
     * @param nbtFile The {@link File} to validate
     ...
     *
     * @param nbtFile The {@link File} to update
    ...
    {
        de.pauleff.jnbt.api.ICompoundTag root = readNBTFile(nbtFile);
     ...
     *
     * @param baseFile The base {@link File}
    ...
    {
        de.pauleff.jnbt.api.ICompoundTag base = readNBTFile(baseFile);
        de.pauleff.jnbt.api.ICompoundTag merge = readNBTFile(mergeFile);
     ...
     */
    private static <T> void updateTagInCompound(de.pauleff.jnbt.api.ICompoundTag compound, String tagPath, T newValue)
    {
        String[] pathParts = tagPath.split("\\.");
        de.pauleff.jnbt.api.ICompoundTag current = compound;
        
        // Navigate to the parent compound
        for (int i = 0; i < pathParts.length - 1; i++)
        {
            de.pauleff.jnbt.api.ICompoundTag next = current.getCompound(pathParts[i]);
            if (next == null)
            {
     ...
     */
    private static void removeTagFromCompound(de.pauleff.jnbt.api.ICompoundTag compound, String tagPath)
    {
        String[] pathParts = tagPath.split("\\.");
        de.pauleff.jnbt.api.ICompoundTag current = compound;
     ...
     */
    private static void mergeCompounds(de.pauleff.jnbt.api.ICompoundTag target, de.pauleff.jnbt.api.ICompoundTag source)
    {
        if (source instanceof Tag_Compound sourceCompound)
        {
            for (Tag<?> sourceTag : sourceCompound.getData())
            {
                if (sourceTag instanceof Tag_Compound && target.hasTag(sourceTag.getName()))
                {
                    ...
                    de.pauleff.jnbt.api.ICompoundTag targetNested = target.getCompound(sourceTag.getName());
